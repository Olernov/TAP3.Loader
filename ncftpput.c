...
static int
Copy(FTPCIPtr cip, const char *dstdir, char **files, int rflag, int xtype, int appendflag, const char *tmppfx, const char *tmpsfx, int resumeflag, int deleteflag, char* ftpResult)
{
	int i;
	int result;
	const char *file;
	...
	for (i=0; ; i++) {
		...
		if (result != 0) {
			(void) sprintf(ftpResult, "ncftpput: file send error: %s.\n", FTPStrError(result));
			rc = result;
		}
	}
	return (rc);
...
//main_void_return_t
int ncftp_main(int argc, char **argv, char* ftpResult)
{
	int result, c;
	int rflag = 0;
	...
	if (result < 0) {
		(void) sprintf(ftpResult, "ncftpput: init library error %d (%s).\n", result, FTPStrError(result));
		DisposeWinsock();
		return (kExitInitLibraryFailed);
	}
	result = FTPInitConnectionInfo(&li, &fi, kDefaultFTPBufSize);
	if (result < 0) {
		(void) sprintf(ftpResult, "ncftpput: init connection info error %d (%s).\n", result, FTPStrError(result));
		DisposeWinsock();
		return (kExitInitConnInfoFailed);
	}
	...
	while ((c = Getopt(&opt, argc, argv, "P:u:p:e:d:U:t:mar:RvVf:AT:S:FcyZzD")) > 0) switch(c) {
		...
		default:
			//Usage();
			sprintf(ftpResult,"Wrong usage of ncftp library");
	}
	if (usingcfg != 0) {
		if (ftpcat == 0) {
			if (opt.ind > argc - 2)
	...
	if ((result = FTPOpenHost(&fi)) < 0) {
		(void) sprintf(ftpResult, "ncftpput: cannot open %s: %s.\n", fi.host, FTPStrError(result));
		es = kExitOpenFailed;
		DisposeWinsock();
		return((int) es);
	}
	if (Umask != NULL) {
		...
		if (result != 0)
			(void) sprintf(ftpResult, "ncftpput: umask failed: %s.\n", FTPStrError(result));
	}
	if (wantMkdir != 0) {
		result = FTPMkdir(&fi, dstdir, kRecursiveYes);
		if (result != 0)
			(void) sprintf(ftpResult, "ncftpput: mkdir failed: %s.\n", FTPStrError(result));
	}
	if (result >= 0) {
		es = kExitXferTimedOut;
		(void) signal(SIGINT, Abort);
		if (ftpcat == 0) {
			if (Copy(&fi, dstdir, files, rflag, xtype, appendflag, (const char *) tmppfx, (const char *) tmpsfx, resumeflag, deleteflag, ftpResult) < 0)
				es = kExitXferFailed;
			else
				es = kExitSuccess;
		} else {
